      ...
      <div className="px-4 md:px-6">
              ...
              >
                <div className="bg-card border shadow-lg rounded-xl p-8 h-full flex flex-col">
                  ...
                  <div className="relative mb-6 flex-1">
                    <Quote className="absolute -top-4 -left-4 h-12 w-12 text-[#C8A96B]/10 rotate-180" />
                    <p className="relative z-10 text-lg font-medium leading-relaxed pl-4 pt-2">
                      {testimonial.content}
                    </p>
                  </div>
        ...
        {trustedCompanies.length > 0 && (
          <motion.div variants={itemVariants} initial="hidden" animate={controls} className="mt-24 text-center">
            <h3 className="text-sm font-medium text-muted-foreground/60 mb-8 font-inter uppercase tracking-widest">{trustedCompaniesTitle}</h3>
            <div className="flex flex-wrap justify-center gap-x-12 gap-y-8">
              {trustedCompanies.map((company) => (
                <div key={company} className="text-xl font-semibold text-muted-foreground/25 font-instrument grayscale opacity-50">
                  {company}
                </div>
              ))}
            </div>
